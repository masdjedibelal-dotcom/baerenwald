...
export function mailPrimaryButtonHtml(label: string, href: string): string {
  return `<p style="margin:20px 0 8px;"><a href="${escAttr(href)}" style="display:inline-block;background:${MAIL_COLORS.primary};color:${MAIL_COLORS.white};text-decoration:none;padding:14px 28px;border-radius:8px;font-weight:600;font-size:15px;">${esc(label)}</a></p>`;
}
...
export function buildStandardMailHtml(opts: BuildStandardMailOpts): string {
  ...
  const disclaimer = opts.disclaimer?.trim()
    ? `<p style="margin:8px 0 0;font-size:11px;color:${MAIL_COLORS.c12};line-height:1.5;">${esc(opts.disclaimer.trim())}</p>`
    : "";
  const footerMain = opts.footerNote?.trim()
    ? `<p style="margin:0;font-size:12px;color:${MAIL_COLORS.gray400};line-height:1.6;">${esc(opts.footerNote.trim())}</p>`
    : `<p style="margin:0;font-size:12px;color:${MAIL_COLORS.gray400};line-height:1.6;">
      Bärenwald München ·
      <a href="${siteUrl}" style="color:${MAIL_COLORS.gray400};text-decoration:none;">${esc(FOOTER_DOMAIN)}</a>
      · ${phoneTxt}
    </p>`;
...
<body style="margin:0;padding:0;background:${MAIL_COLORS.white};font-family:Arial,Helvetica,sans-serif;-webkit-text-size-adjust:100%;color-scheme:light;">
${pre}
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:${MAIL_COLORS.white};">
<tr><td align="center" style="padding:32px 16px;background:${MAIL_COLORS.white};">
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:580px;width:100%;background:${MAIL_COLORS.white};">

<tr>
  <td style="padding:0 0 24px;border-bottom:1px solid ${MAIL_COLORS.gray200};">
    ...
    <tr>
      ...
      <td style="vertical-align:middle;font-size:20px;font-weight:700;color:${MAIL_COLORS.primaryDk};letter-spacing:-0.02em;line-height:1;">
        Bärenwald
      </td>
    </tr>
    </table>
...
<tr>
  <td style="padding:32px 0 24px;background:${MAIL_COLORS.white};">
    ${opts.bodyHtml}
  </td>
</tr>

<tr>
  <td style="padding:20px 0 0;border-top:1px solid ${MAIL_COLORS.gray200};background:${MAIL_COLORS.white};">
    ${footerMain}
    ${disclaimer}
  </td>
</tr>
