/** Fehler / System — Sie-Form. Technik nur ins Log. */
...
export const COPY_ERROR = {
  ...
  keineAnnahme: "Keine Annahme möglich.",
} as const;
...
export function isLikelyOfflineError(error?: unknown): boolean {
  ...
  return /failed to fetch|networkerror|netzwerk|offline|econn|etimedout|enotfound|load failed|internet/i.test(
    raw
  );
}
