...
export function resolvePartnerOnboardingGaps(input: {
  ...
  else if (missingFirmendaten.some((m) => /Anschrift/i.test(m))) {
    preferredTab = "anschrift";
  } else if (missingFirmendaten.some((m) => /Steuer|USt/i.test(m))) {
    preferredTab = "steuer";
  } else if (missingFirmendaten.some((m) => /IBAN|Bank/i.test(m))) {
    preferredTab = "bank";
  }
