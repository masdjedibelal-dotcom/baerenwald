/**
 * Lokaler Zwischenstand für lange Portal-Formulare (localStorage).
...
/** Entfernt File/Blob; kürzt riesige data:-URLs. */
...
export function readZwischenstand<T>(key: string): Envelope<T> | null {
  ...
  try {
    ...
    if (!parsed || parsed.v !== 1 || !parsed.savedAt || parsed.data == null)
      return null;
    return parsed;
  } catch {
    return null;
