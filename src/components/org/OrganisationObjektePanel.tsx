...
export function OrganisationObjektePanel({ objekte, orgKennung, onRefresh }: Props) {
  ...
  const [schwelleEdit, setSchwelleEdit] = useState<Record<string, string>>({});
          ...
          edit?.id === o.id ? (
            ...
            <li key={o.id} className="card-bordered p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                >
                  {expandedId === o.id ? "Schließen" : "Objektakte"}
                </button>
              </div>
              {expandedId === o.id ? (
                <>
                  <OrganisationObjektAkteTabs objektId={o.id} />
                  <div className="mt-4 space-y-2 border-t border-border-default pt-4">
                    <p className="portal-text-label">Freigabeschwelle für dieses Objekt</p>
                    <p className="portal-text-meta text-text-tertiary">
                      Standard der Verwaltung: 2.500 € — hier abweichend festlegbar (z. B. WEG-Beschluss).
                    </p>
                    <div className="flex gap-2">
                      <input
                        type="number"
                        ...
                        value={schwelleEdit[o.id] ?? ""}
                        onChange={(e) =>
                          setSchwelleEdit((s) => ({ ...s, [o.id]: e.target.value }))
                        }
                      />
                      <button
                        type="button"
                        ...
                        onClick={async () => {
                          const v = schwelleEdit[o.id]?.trim();
