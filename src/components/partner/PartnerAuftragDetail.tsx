...
export function PartnerAuftragDetail({
  ...
  const [showHwKalk, setShowHwKalk] = useState(false);
  ...
  return (
    <PartnerDetailLayout>
      ...
      {(() => {
        ...
        if (!anfrageId) return null;
        const hwSt = (item.angebotHwStatus ?? "").toLowerCase();
        const alreadySubmitted =
          Boolean(item.angebotHwEingereichtAt) ||
          hwSt === "eingereicht" ||
          hwSt === "uebernommen";
        if (alreadySubmitted) {
          return (
            <PartnerDetailSection title="Kalkulation / Angebot">
              <PartnerDetailSuccessBox>
                <p className="font-semibold">Angebot eingereicht</p>
                <p className="portal-text-meta mt-1 text-text-secondary">
                  Sichtbar im CRM und bei der Hausverwaltung als empfohlenes Angebot.
                  {item.angebotHwEingereichtAt
                    ? ` · ${fmtPartnerDate(item.angebotHwEingereichtAt)}`
                    : ""}
                </p>
              </PartnerDetailSuccessBox>
            </PartnerDetailSection>
        ...
        return (
          <PartnerDetailSection title="Kalkulation / Angebot">
            <p className="portal-text-body text-text-secondary mb-3">
              Positionen kalkulieren und einreichen — erscheint als empfohlenes
              Angebot bei der Hausverwaltung.
            </p>
            <button
              type="button"
              className="btn-pill-primary portal-btn"
              onClick={() => setShowHwKalk(true)}
            >
              Kalkulation öffnen
            </button>
          </PartnerDetailSection>
