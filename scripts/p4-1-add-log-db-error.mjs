...
function processFile(filePath) {
  ...
  const ops = []
  let fileSeq = 0
  const nextAlias = () => `__dbErr${++globalErrSeq}_${++fileSeq}`
  ...
  while ((m = reDestructure.exec(src))) {
    ...
    if (/\berror\b/.test(braces)) {
      // Bestehende error-Bindings nie umbenennen — nur loggen
      const errName =
  ...
  while ((m = reFire.exec(src))) {
    ...
    const replacement =
      `const { error: ${alias} } = await ${awaitExpr}\n` +
      `${ind}if (${alias}) logDbError('${ctx}', ${alias})`
    ops.push({
      kind: 'wrap-fire',
  ...
  for (const op of uniq) {
    ...
    if (op.kind === 'add-error') {
      ...
      const aliasPart = `error: ${op.alias}`
      const newBraces = bracesContent
        ? bracesContent.endsWith(',')
          ? `${bracesContent} ${aliasPart}`
          : `${bracesContent}, ${aliasPart}`
        : aliasPart
      ...
      const logLine = `\n${op.ind}if (${op.alias}) logDbError('${op.ctx}', ${op.alias})`
